...
### Batched Training Loop (Work in Progress)
`uv run python batched_training_loop.py`
This script introduces a preliminary implementation of a batched training loop. It aims to process multiple image pairs concurrently. **Note:** This implementation currently exhibits high VRAM usage (~30GB for a batch of 4 items), indicating a potential memory leak or inefficient handling of tensors. Further investigation and optimization are required.
