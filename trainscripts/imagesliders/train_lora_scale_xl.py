...
def parse_adapters(adapter_string: str) -> list[tuple[str, float]]:
    """Parses the --base_adapters argument string."""
    if not adapter_string:
        return []
    
    adapters = []
    for part in adapter_string.split(','):
        path, scale_str = part.strip().split(':')
        adapters.append((path, float(scale_str)))
    return adapters
...
def train(
    ...
    base_adapter_list = parse_adapters(base_adapters_str)
    if base_adapter_list:
        # This function modifies the UNet's weights directly.
        lora_alt.apply_base_adapters_reparam(unet, base_adapter_list)
        print("Base adapters have been merged into the UNet for this run. Good luck.")
...
def main(args):
    ...
    scales = args.scales
    base_adapters_str = args.base_adapters
    ...
    if args.stylecheck is not None:
        ...
        train(config=config, prompts=prompts, device=device, folder_main = args.folder_main, folders = folders, scales = scales, base_adapters_str = base_adapters_str)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="Pre-trained adapters to fuse before training. Format: 'path1:scale1,path2:scale2,...'",
    )
