...
def superfunctional_train_step(
    ...
    with network:
        target_latents_low_batch = train_util.predict_noise_xl(
            ...
            noise_scheduler,
            current_timestep,
            torch.cat([batched_denoised_latents_low, batched_denoised_latents_low], dim=0),
            torch.cat([batched_neutral_text_embeds, batched_neutral_text_embeds], dim=0), # These should be duplicated
            torch.cat([batched_neutral_pooled_embeds, batched_neutral_pooled_embeds], dim=0), # These should be duplicated
            torch.cat([batched_add_time_ids_low, batched_add_time_ids_low], dim=0), # These should be duplicated
            guidance_scale=1,
        )
